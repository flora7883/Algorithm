...
class Edge_Beakjoon_1414{
	int s;
	int e;
	int w;
	Edge_Beakjoon_1414(int s, int e, int w){
		this.s=s;
		this.e=e;
		this.w=w;
	}
...
public class Beakjoon_1414 {
	static int N;
	static PriorityQueue<Edge_Beakjoon_1414> pq;
	static int[] parent;
			...
			for(int j=0;j<N;j++) {
				...
				if(c!='0') {
					...
					if('a'<= c && c<='z')
						...
						w=c-'A'+1+26;
					pq.add(new Edge_Beakjoon_1414(i,j,w));
					sum+=w;
				}
	...
	public static int minimun_spanning_tree() {
		...
		int result=0;
		while(!pq.isEmpty()) {
			Edge_Beakjoon_1414 edge=pq.poll();
