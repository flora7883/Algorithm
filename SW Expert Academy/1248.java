...
class BinaryTree{
	...
	void insert(int p, int c) {
		...
		if(node[p]==null) {
			node[p]=new_node(p);
			//System.out.printf("parent null %d\n", node[p].num);
		}
		if(node[c]==null) {
			node[c]=new_node(c);
			//System.out.printf("child null %d\n", node[c].num);
		}
		if(node[p].left==null) {
			node[p].left=node[c];
			//System.out.printf("left연결 %d\n", node[p].left.num);
		}
		else {
			node[p].right=node[c];
			//System.out.printf("right연결 %d\n", node[p].right.num);
		}
		node[c].parent=node[p];
		//System.out.printf("부모연결 %d\n", node[c].parent.num);
	}
...
class Solution
{
	public static void main(String args[]) throws Exception
	{
		...
		for(int test_case = 1; test_case <= T; test_case++)
		{
			...
			System.out.printf("#%d %d %d",test_case, root, tree.size(root));
		}
	}
}
